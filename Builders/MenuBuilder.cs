...
namespace MenuBuilder
{
    class MenuBuilder : IDatabaseBuilder
    {
        ...
        {
            //TODO set use Date format
            _logger(new DatabaseMessage() { MessageType = DatabaseMessage.BuilderMessageType.Information, Message = "Menu build ended " + DateTime.Now.ToString() });
            ...
            var doc = new XDocument(new XDeclaration("1.0", "UTF-8", "true"),
                new XElement("Menu",
                    ...
                    select new XElement("Language",
                             ...
                             new XAttribute("default", lang == _defaultLanguage),
            ...
            else if (item is PCAxis.Paxiom.PXMeta)
            {
                ...
                foreach (var language in _languages)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (meta.HasLanguage(_defaultLanguage))
                            {
                                meta.SetLanguage(_defaultLanguage);
                            }
                            else
                            {
